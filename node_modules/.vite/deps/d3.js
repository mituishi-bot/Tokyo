...
function boundsRingEnd() {
  ...
  areaStream.lineEnd();
  if (abs3(deltaSum) > epsilon3) lambda02 = -(lambda1 = 180);
  range3[0] = lambda02, range3[1] = lambda1;
  p0 = null;
}
...
function centroidRingPoint(lambda, phi2) {
  lambda *= radians, phi2 *= radians;
  var cosPhi = cos2(phi2), x3 = cosPhi * cos2(lambda), y3 = cosPhi * sin2(lambda), z = sin2(phi2), cx = y0 * z - z0 * y3, cy = z0 * x3 - x0 * z, cz = x0 * y3 - y0 * x3, m3 = hypot(cx, cy, cz), w = asin(m3), v = m3 && -w / m3;
  X2.add(v * cx);
  Y2.add(v * cy);
  Z2.add(v * cz);
  W1 += w;
...
function rotationIdentity(lambda, phi2) {
  if (abs3(lambda) > pi3) lambda -= Math.round(lambda / tau3) * tau3;
  return [lambda, phi2];
}
...
function circleRadius(cosRadius, point2) {
  ...
  return ((-point2[2] < 0 ? -radius : radius) + tau3 - epsilon3) % tau3;
}
...
function pointEqual_default(a3, b) {
  return abs3(a3[0] - b[0]) < epsilon3 && abs3(a3[1] - b[1]) < epsilon3;
}
...
function compareIntersection(a3, b) {
  return ((a3 = a3.x)[0] < 0 ? a3[1] - halfPi3 - epsilon3 : halfPi3 - a3[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi3 - epsilon3 : halfPi3 - b[1]);
}
  ...
  return {
    ...
    point: function(lambda12, phi12) {
      ...
      if (abs3(delta - pi3) < epsilon3) {
        ...
        if (abs3(lambda04 - sign0) < epsilon3) lambda04 -= sign0 * epsilon3;
        if (abs3(lambda12 - sign1) < epsilon3) lambda12 -= sign1 * epsilon3;
...
function clipAntimeridianIntersect(lambda04, phi02, lambda12, phi12) {
  ...
  return abs3(sinLambda0Lambda1) > epsilon3 ? atan((sin2(phi02) * (cosPhi1 = cos2(phi12)) * sin2(lambda12) - sin2(phi12) * (cosPhi03 = cos2(phi02)) * sin2(lambda04)) / (cosPhi03 * cosPhi1 * sinLambda0Lambda1)) : (phi02 + phi12) / 2;
}
...
function circle_default3(radius) {
  var cr = cos2(radius), delta = 2 * radians, smallRadius = cr > 0, notHemisphere = abs3(cr) > epsilon3;
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi2) {
        var point1 = [lambda, phi2], point2, v = visible(lambda, phi2), c3 = smallRadius ? v ? 0 : code(lambda, phi2) : v ? code(lambda + (lambda < 0 ? pi3 : -pi3), phi2) : 0;
  ...
  function intersect(a3, b, two) {
    ...
    var delta2 = lambda12 - lambda04, polar = abs3(delta2 - pi3) < epsilon3, meridian = polar || delta2 < epsilon3;
    if (!polar && phi12 < phi02) z = phi02, phi02 = phi12, phi12 = z;
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs3(q[0] - lambda04) < epsilon3 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi3 ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function clipRectangle(x06, y06, x12, y12) {
  ...
  function corner(p, direction) {
    return abs3(p[0] - x06) < epsilon3 ? direction > 0 ? 0 : 3 : abs3(p[0] - x12) < epsilon3 ? direction > 0 ? 2 : 1 : abs3(p[1] - y06) < epsilon3 ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
  }
...
function interpolate_default2(a3, b) {
  ...
  var interpolate = d ? function(t) {
    var B = sin2(t *= d) / k, A = sin2(d - t) / k, x3 = A * kx0 + B * kx1, y3 = A * ky0 + B * ky1, z = A * sy0 + B * sy1;
    return [
      atan2(y3, x3) * degrees,
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b02 + b12, c3 = c0 + c1, m3 = sqrt2(a3 * a3 + b * b + c3 * c3), phi2 = asin(c3 /= m3), lambda22 = abs3(abs3(c3) - 1) < epsilon3 || abs3(lambda04 - lambda12) < epsilon3 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b02 * b12 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b02, c0, x22, y22, lambda22, a3 /= m3, b /= m3, c3, depth, stream);
        stream.point(x22, y22);
        resampleLineTo(x22, y22, lambda22, a3, b, c3, x12, y12, lambda12, a1, b12, c1, depth, stream);
      }
    }
  }
  return function(stream) {
    ...
    function linePoint2(lambda, phi2) {
      ...
      resampleLineTo(x06, y06, lambda04, a0, b02, c0, x06 = p[0], y06 = p[1], lambda04 = lambda, a0 = c3[0], b02 = c3[1], c0 = c3[2], maxDepth, stream);
...
function conicEqualAreaRaw(y06, y12) {
  ...
  if (abs3(n) < epsilon3) return cylindricalEqualAreaRaw(y06);
...
function azimuthalRaw(scale) {
  return function(x3, y3) {
    var cx = cos2(x3), cy = cos2(y3), k = scale(cx * cy);
...
function azimuthalInvert(angle2) {
  return function(x3, y3) {
    var z = sqrt2(x3 * x3 + y3 * y3), c3 = angle2(z), sc = sin2(c3), cc = cos2(c3);
    return [
      atan2(x3 * sc, z * cc),
      asin(z && y3 * sc / z)
    ];
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k = pi3 * scale(), t = m3(rotation_default(m3.rotate()).invert([0, 0]));
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x3, y3) {
    if (f > 0) {
      if (y3 < -halfPi3 + epsilon3) y3 = -halfPi3 + epsilon3;
    } else {
      if (y3 > halfPi3 - epsilon3) y3 = halfPi3 - epsilon3;
    }
...
function conicEquidistantRaw(y06, y12) {
  ...
  if (abs3(n) < epsilon3) return equirectangularRaw;
  function project(x3, y3) {
    var gy = g - y3, nx = n * x3;
...
function encloseBasis3(a3, b, c3) {
  var x12 = a3.x, y12 = a3.y, r1 = a3.r, x22 = b.x, y22 = b.y, r2 = b.r, x3 = c3.x, y3 = c3.y, r3 = c3.r, a22 = x12 - x22, a32 = x12 - x3, b22 = y12 - y22, b32 = y12 - y3, c22 = r2 - r1, c32 = r3 - r1, d1 = x12 * x12 + y12 * y12 - r1 * r1, d2 = d1 - x22 * x22 - y22 * y22 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab = a32 * b22 - a22 * b32, xa = (b22 * d3 - b32 * d2) / (ab * 2) - x12, xb = (b32 * c22 - b22 * c32) / ab, ya = (a32 * d2 - a22 * d3) / (ab * 2) - y12, yb = (a22 * c32 - a32 * c22) / ab, A = xb * xb + yb * yb - 1, B = 2 * (r1 + xa * xb + ya * yb), C = xa * xa + ya * ya - r1 * r1, r = -(Math.abs(A) > 1e-6 ? (B + Math.sqrt(B * B - 4 * A * C)) / (2 * A) : C / B);
  return {
    x: x12 + xa + xb * r,
    y: y12 + ya + yb * r,
    r
  ...
  function randomBinomial(n, p) {
    ...
    return function() {
      ...
      while (nn * pp > 16 && nn * (1 - pp) > 16) {
        var i = Math.floor((nn + 1) * pp), y3 = B(i, nn - i + 1)();
        if (y3 <= pp) {
          acc += i;
          nn -= i;
          pp = (pp - y3) / (1 - y3);
...
function zoom_default2() {
  ...
  function dblclicked(event, ...args) {
    if (!filter3.apply(this, arguments)) return;
    var t0 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t0.invert(p02), k1 = t0.k * (event.shiftKey ? 0.5 : 2), t1 = constrain(translate(scale(t0, k1), p02, p1), extent2.apply(this, args), translateExtent);
    noevent_default3(event);
